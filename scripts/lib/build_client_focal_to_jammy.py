#!/usr/bin/env python3
"""Build the single-file Focal→Jammy offline DP upgrade client artifact.
...
KEYRING_INSTALL_PATH = "/etc/apt/trusted.gpg.d/stellar-offline-focal-to-jammy.gpg"
...
FORBIDDEN_B2F_RESIDUAL_RE = re.compile(
    r"UPGRADE-BIONIC-TO-FOCAL|UPGRADING_BIONIC_TO_FOCAL|PREPARING_BIONIC|"
    r"COMPLETED_BIONIC|stellar-offline-bionic-to-focal|"
    r"dp-offline-upgrade-bionic-to-focal|xenial-to-bionic|"
    r"PIN_HOP=.bionic-to-focal.|HOP = .bionic-to-focal.",
    re.I,
)
...
def build_meta_release_lts(mirror_base, hop, announcement_name="ReleaseAnnouncement"):
    """Build local-only meta-release-lts for Focal to Jammy (no Canonical hosts).

    Client-generated /etc/update-manager/meta-release INI must stay ASCII-only for
    Focal Python 3.8 configparser under POSIX locale. Dist-format bodies here may
    ...
    focal = OrderedDict(
        [
            ...
            (
                "UpgradeTool",
                "{}/offline/release-upgraders/jammy/jammy.tar.gz".format(mb),
            ),
            (
                "UpgradeToolSignature",
                "{}/offline/release-upgraders/jammy/jammy.tar.gz.gpg".format(mb),
            ),
        ]
    )
    jammy = OrderedDict(
...
def assert_no_b2f_residuals(label, body):
...
def main(argv=None):
    ...
    ap.add_argument(
        "--template",
        default="",
        help="default: client/dp-offline-upgrade-focal-to-jammy.sh.in",
    )
    ap.add_argument(
        "--deploy-nginx-root",
        default="",
    ...
    template = args.template or os.path.join(
        project_root, "client", "dp-offline-upgrade-focal-to-jammy.sh.in"
    )
    ...
    manifest = OrderedDict(
        [
            ...
            (
                "upgrader_tar_url",
                "{}/offline/release-upgraders/jammy/jammy.tar.gz".format(mirror_base),
            ),
            ("upgrader_tar_sha256", up_tar_sha),
            (
                "upgrader_gpg_url",
                "{}/offline/release-upgraders/jammy/jammy.tar.gz.gpg".format(
                    mirror_base
                ),
            ),
    ...
    print("BUILD_CLIENT_FOCAL_TO_JAMMY PASS")
...
if __name__ == "__main__":
    try:
        ...
        print("BUILD_CLIENT_FOCAL_TO_JAMMY FAIL: {}".format(exc), file=sys.stderr)
        sys.exit(1)
