#!/usr/bin/env python3
"""Build the single-file Jammy→Noble offline DP upgrade client artifact.
...
KEYRING_INSTALL_PATH = "/etc/apt/trusted.gpg.d/stellar-offline-jammy-to-noble.gpg"
...
# Forbid F2J *current-hop* literals leaking into J2N artifacts.
# Allowed: PREVIOUS_HOP_PIN_NAME=focal-to-jammy / PREVIOUS_HOP_TERMINAL_STATE=COMPLETED_JAMMY.
FORBIDDEN_F2J_RESIDUAL_RE = re.compile(
    r"UPGRADE-FOCAL-TO-JAMMY|UPGRADING_FOCAL_TO_JAMMY|PREPARING_FOCAL|"
    r"COMPLETED_FOCAL|stellar-offline-focal-to-jammy|"
    r"dp-offline-upgrade-focal-to-jammy|"
    r"PIN_HOP=.focal-to-jammy.|HOP = .focal-to-jammy.|"
    r"CURRENT_HOP=focal-to-jammy|CURRENT_TARGET_CODENAME=jammy|"
    r"jammy/jammy\.tar\.gz|focal/focal\.tar\.gz|"
    r"bionic-to-focal|xenial-to-bionic",
    re.I,
)
...
def build_meta_release_lts(mirror_base, hop, announcement_name="ReleaseAnnouncement"):
    """Build local-only meta-release-lts for Jammy to Noble (no Canonical hosts).

    Client-generated /etc/update-manager/meta-release INI must stay ASCII-only for
    Jammy Python 3.8 configparser under POSIX locale. Dist-format bodies here may
    ...
    jammy = OrderedDict(
        [
            ...
            ("Supported", "1"),
            ("Description", "This is the 22.04.6 LTS release"),
            ("Release-File", "{}/dists/jammy/Release".format(hop_ubuntu)),
            (
                "ReleaseNotes",
            ...
            (
                "UpgradeTool",
                "{}/offline/release-upgraders/noble/noble.tar.gz".format(mb),
            ),
            (
                "UpgradeToolSignature",
                "{}/offline/release-upgraders/noble/noble.tar.gz.gpg".format(mb),
            ),
        ]
    )
    noble = OrderedDict(
        [
            ("Dist", "noble"),
            ("Name", "Noble Jellyfish"),
            ("Version", "24.04.5 LTS"),
            ("Date", "Thu, 21 April 2022 22:04:00 UTC"),
            ("Supported", "1"),
            ("Description", "This is the 24.04.5 LTS release"),
            ("Release-File", "{}/dists/noble/Release".format(hop_ubuntu)),
            (
                "ReleaseNotes",
                "{}/client/{}/{}".format(mb, hop, announcement_name),
            ),
...
def assert_no_f2j_residuals(label, body):
...
def main(argv=None):
    ...
    ap.add_argument(
        "--template",
        default="",
        help="default: client/dp-offline-upgrade-jammy-to-noble.sh.in",
    )
    ap.add_argument(
        "--deploy-nginx-root",
        default="",
    ...
    template = args.template or os.path.join(
        project_root, "client", "dp-offline-upgrade-jammy-to-noble.sh.in"
    )
    ...
    upgrader_tar = resolve_upgrader_tar(selective_root, "noble")
    ...
    manifest = OrderedDict(
        [
            ...
            (
                "upgrader_tar_url",
                "{}/offline/release-upgraders/noble/noble.tar.gz".format(mirror_base),
            ),
            ("upgrader_tar_sha256", up_tar_sha),
            (
                "upgrader_gpg_url",
                "{}/offline/release-upgraders/noble/noble.tar.gz.gpg".format(
                    mirror_base
                ),
            ),
    ...
    script_name = "dp-offline-upgrade-jammy-to-noble.sh"
    ...
    print("BUILD_CLIENT_JAMMY_TO_NOBLE PASS")
...
if __name__ == "__main__":
    try:
        ...
        print("BUILD_CLIENT_JAMMY_TO_NOBLE FAIL: {}".format(exc), file=sys.stderr)
        sys.exit(1)
