...
# Menu 7 production viewer: framed dialog --textbox; no clear/less/raw viewer.
fn="$(awk '/^mm_menu7_textbox\(\)/,/^}/' "$INSTALLER")"
printf '%s\n' "$fn" | grep -qE '(^|[^A-Za-z_])dialog([^A-Za-z_]|$)' \
  && printf '%s\n' "$fn" | grep -q -- '--textbox' \
  || fail "mm_menu7_textbox missing dialog --textbox"
printf '%s\n' "$fn" | grep -q -- '--no-mouse' \
  || fail "Menu 7 missing --no-mouse"
printf '%s\n' "$fn" | grep -q 'menu7_scroll_viewer.py' \
  && fail "Menu 7 still uses raw terminal scroll viewer" || true
