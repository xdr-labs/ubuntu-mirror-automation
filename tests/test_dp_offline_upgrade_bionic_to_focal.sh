#!/usr/bin/env bash
# tests/test_dp_offline_upgrade_bionic_to_focal.sh
set -euo pipefail
...
SCRIPT_IN_RAW="${ROOT}/client/dp-offline-upgrade-bionic-to-focal.sh.in"
...
STUB="${OUT_DIR}/dp-offline-upgrade-bionic-to-focal.stub.sh"
...
pins = {
    ...
    "TARGET_SUITES": "focal focal-updates focal-security focal-backports",
...
# Wrong OS → FAIL
rm -rf "$fx2/opt/aelladata/os-upgrade"
sed -i 's/18.04/20.04/;s/bionic/focal/' "$fx2/etc/os-release"
rc="$(run_preflight_fixture "$fx2")"
...
run_commit_fixture() {
  ...
  rm -f "$root/etc/apt/trusted.gpg.d/stellar-offline-bionic-to-focal.gpg"
...
parts = [
    ...
    'PIN_TARGET_SUITES="focal focal-updates focal-security focal-backports"',
    'MIRROR_BASE="http://127.0.0.1:9"',
...
open(p, "wb").write(b"\xef\xbb\xbf# bionic \xe2\x86\x92 bionic\n" + b"".join(deb))
PY
set +e
...
printf 'deb http://ppa.example/ubuntu bionic main\n' >"$fx_rb/etc/apt/sources.list.d/example-ppa.list"
...
# Simulate mutation
printf 'deb [arch=amd64] http://offline/ubuntu bionic main\n' >"$fx_rb/etc/apt/sources.list"
...
is_focal_version_for_pkg() { return 1; }
assert_ascii_file() {
  local path="$1"
...
printf '# Managed by stellar offline bionic\xe2\x86\x92bionic upgrade\n[METARELEASE]\nURI = file:///tmp/x\nURI_LTS = file:///tmp/x\n' \
...
# 2) ASCII bionic-to-focal -> PASS
meta_local="$fx_meta/opt/aelladata/os-upgrade/offline/meta-release-lts.runtime"
printf 'Dist: bionic\nName: Ubuntu 18.04 LTS\n' >"$meta_local"
render_update_manager_meta_release "$meta_local" "$fx_meta/etc/update-manager/meta-release.good" "bionic-to-focal"
if assert_ascii_file "$fx_meta/etc/update-manager/meta-release.good"; then
  pass "ASCII bionic-to-focal meta-release -> PASS"
else
  fail "ASCII meta-release rejected"
fi
if grep -q 'bionic-to-focal' "$fx_meta/etc/update-manager/meta-release.good" \
...
# 12) OS 18.04 + clean -> partial NO
PARTIAL_RELEASE_TRANSITION="YES"
if ! package_transition_evidence_present; then
  PARTIAL_RELEASE_TRANSITION="NO"
  pass "OS 18.04 clean dpkg -> partial transition NO"
else
...
if grep -n "Managed by stellar offline bionic" "$SCRIPT_IN" | grep -q $'\xe2'; then
...
if [[ -f "${SEL_ROOT}/keys/ubuntu-mirror-selective.gpg" \
  ...
  if [[ -f "${SEL_ROOT}/hops/bionic-to-focal/ubuntu/dists/bionic/Release" ]] \
    || [[ -f "${SEL_ROOT}/current/hops/bionic-to-focal/ubuntu/dists/bionic/Release" ]]; then
    set +e
    python3 "$BUILD_PY" \
      --project-root "$ROOT" \
    ...
    if [[ "$rc" -eq 0 ]]; then
      ...
      grep -q 'UPGRADE-BIONIC-TO-FOCAL' "$BUILT" && pass "confirm phrase pinned" || fail "confirm phrase"
      grep -q 'DistUpgradeViewNonInteractive' "$BUILT" && pass "DRO frontend pinned" || fail "DRO frontend"
      grep -q 'COMPLETED_FOCAL' "$BUILT" && pass "COMPLETED_FOCAL state" || fail "COMPLETED_FOCAL"
      ...
      disc_sha="$(sha256sum "${ROOT}/artifacts/upgrade-discovery/bionic-to-focal/export-summary.json" | awk '{print $1}')"
      sleep 0
      disc_sha2="$(sha256sum "${ROOT}/artifacts/upgrade-discovery/bionic-to-focal/export-summary.json" | awk '{print $1}')"
...
EOF
  ...
  printf 'deb http://ppa.launchpad.net/example/ppa/ubuntu bionic main\n' \
...
EOF
  ...
  if [[ ! -f "$fake/etc/apt/keyrings/stellar-offline-upgrade.gpg" \
     && ! -f "$fake/etc/apt/trusted.gpg.d/stellar-offline-bionic-to-focal.gpg" ]]; then
  ...
  if [[ -f "$fake/etc/apt/trusted.gpg.d/stellar-offline-bionic-to-focal.gpg" ]] \
    ...
    if [[ -f "$fake/etc/update-manager/meta-release" ]] \
       ...
       && grep -q 'bionic-to-focal' "$fake/etc/update-manager/meta-release" \
...
EOF
  ...
  # Refuse 22.04+ (Focal is this hop's target; later hops are out of scope)
  cat >"$fake/etc/os-release" <<EOF
NAME="Ubuntu"
VERSION_ID="22.04"
...
# 12.4 state PREPARING_BIONIC → PASS
hf="$(mktemp -d)"
make_handoff_fixture "$hf"
install_fake_systemctl "$hf" ok
rpid="$(spawn_fake_runner "$hf")"
printf 'PREPARING_BIONIC\n' >"$hf/opt/aelladata/os-upgrade/offline/state"
export DP_OFFLINE_TEST_ROOT="$hf" DP_OFFLINE_TEST_HANDOFF=1 TEST_ROOT="$hf" STELLAR_OFFLINE_TEST_ROOT="$hf"
...
[[ "$rc" -eq 0 ]] && grep -q 'UPGRADE_HANDOFF=PASS' "$hf/out-prep.txt" \
  && pass "state PREPARING_BIONIC → handoff PASS" \
  || { fail "PREPARING_BIONIC handoff"; cat "$hf/out-prep.txt"; }
kill_fake_runner "$hf"; rm -rf "$hf"
...
if _hop_buf_has_f 'Type=oneshot' "$_unit_blk" \
  ...
  _hop_tmp="$(_hop_capture grep -A25 'Description=Stellar offline OS upgrade Bionic to Focal' "$SCRIPT_IN")"
...
if [[ "$rc" -eq 0 ]] \
   ...
   && grep -q 'UPGRADING_BIONIC_TO_FOCAL' "$hf/out-mon.txt" \
...
printf 'deb http://example/ubuntu bionic main\n' >"$hf/etc/apt/sources.list"
...
PIN_TARGET_VERSION='20.04'
PIN_COMPONENTS='main universe'
PIN_SOURCE_SUITES='bionic bionic-updates bionic-security bionic-backports'
PIN_TARGET_SUITES='focal focal-updates focal-security focal-backports'
...
if grep -q 'APT_SOURCES_LIST_RESTORED=PASS' "$logf" \
  ...
  && grep -q 'deb http://example.invalid/ bionic main' "$rf/etc/apt/sources.list"; then
...
st="$(cat "$rf/opt/aelladata/os-upgrade/offline/state")"
[[ "$st" == "FAILED_PRE_DRO" ]] && pass "pre-DRO fail state=FAILED_PRE_DRO (not PREPARING_BIONIC)" \
...
# 13.10 stale PREPARING_BIONIC without live evidence → not MONITOR_ONLY
sf="$(mktemp -d)"
...
if grep -q 'STALE_STATE_DETECTED=YES' "$sf/out-stale.txt" \
  ...
  pass "stale PREPARING_BIONIC diagnosed; not MONITOR_ONLY"
else
  fail "stale PREPARING_BIONIC handling"
...
printf '# stellar offline DistUpgrade ValidMirrors overlay (Bionic-to-Focal)\nhttp://archive.ubuntu.com/ubuntu/\n' \
  >"$rf/opt/aelladata/os-upgrade/offline/distupgrade-valid-mirrors.cfg"
printf '# Generated by stellar offline Bionic-to-Focal client.\n[Sources]\nValidMirrors=%s\nAllowThirdParty=yes\n' \
  "$rf/opt/aelladata/os-upgrade/offline/distupgrade-valid-mirrors.cfg" \
...
if [[ "$st" == "FAILED_BEFORE_PACKAGE_TRANSITION" ]] \
  ...
  && grep -q 'deb http://example.invalid/ bionic main' "$rf/etc/apt/sources.list" \
...
if [[ "$rc" -eq 0 ]] && printf '%s\n' "$out" | grep -q 'DISTUPGRADE_EFFECTIVE_SOURCE_GATE_STATE=DISARMED' \
  ...
  pass "DISARMED gate allows Bionic (NOT_ARMED)"
else
  fail "DISARMED gate failed (rc=${rc})"
  printf '%s\n' "$out" || true
fi

# Arm marker + Bionic only → DEFER
cat >"$GATE_FIX/opt/aelladata/os-upgrade/offline/effective-source-gate.armed" <<EOF
GATE_SCHEMA_VERSION=1
EXPECTED_SOURCE_CODENAME=bionic
EXPECTED_TARGET_CODENAME=focal
EXPECTED_MIRROR_URI=${LOCAL_URI}
EXPECTED_HOP=bionic-to-focal
...
if [[ "$rc" -eq 0 ]] && printf '%s\n' "$out" | grep -q 'ARMED_WAITING_FOR_TARGET_REWRITE' \
  ...
  pass "ARMED+Bionic defers (TARGET_REWRITE_NOT_YET_VISIBLE)"
else
  fail "ARMED defer failed (rc=${rc})"
  printf '%s\n' "$out" || true
fi

# Arm + local Focal → PASS + passed marker
...
if [[ "$rc" -eq 0 ]] \
  ...
  pass "ARMED+local Focal strict PASS + passed marker"
else
  fail "ARMED Focal PASS failed (rc=${rc})"
  printf '%s\n' "$out" || true
fi
...
# Arm + country mirror
cat >"$GATE_FIX/etc/apt/sources.list" <<EOF
deb [arch=amd64] http://kr.archive.ubuntu.com/ubuntu focal main universe
deb [arch=amd64] ${LOCAL_URI} focal-updates main universe
deb [arch=amd64] ${LOCAL_URI} focal-security main universe
deb [arch=amd64] ${LOCAL_URI} focal-backports main universe
EOF
...
# Duplicate
write_focal_sources "$GATE_FIX/etc/apt/sources.list"
echo "deb [arch=amd64] ${LOCAL_URI} focal main universe" >>"$GATE_FIX/etc/apt/sources.list"
set +e
out="$(run_gate "$GATE_FIX/etc/apt/sources.list" 2>/dev/null)"
rc=$?
set -e
...
# Arm only now (DRO eve), still Bionic sources → next Pre-Invoke would DEFER.
...
# Simulate Pre-Invoke after arm with Bionic sources still in place.
...
# =============================================================================
# 17) Previous-hop (COMPLETED_BIONIC) multi-hop handoff
# =============================================================================
...
grep -q 'BIONIC_TO_FOCAL_HANDOFF_ACCEPTED=YES' "$SCRIPT_IN" \
  && pass "BIONIC_TO_FOCAL_HANDOFF_ACCEPTED marker" || fail "handoff accepted marker missing"
_hop_tmp="$(_hop_capture grep -A3 'COMPLETED_BIONIC)' "$SCRIPT_IN")"
if _hop_buf_has_f 'accept_previous_hop_terminal_state' "$_hop_tmp"; then
  pass "COMPLETED_BIONIC routes through accept_previous_hop_terminal_state"
else
  fail "COMPLETED_BIONIC not routed through handoff accept"
fi
...
# 17.A normal inherited COMPLETED_BIONIC → handoff → preflight
...
if [[ "$rc" -eq 0 ]] \
  ...
  && grep -q 'PREVIOUS_HOP_STATE=COMPLETED_BIONIC' "$hop_a/out.txt" \
  && ! grep -q "unrecognized/corrupt state='COMPLETED_BIONIC'" "$hop_a/out.txt" \
  ...
  pass "17.A inherited COMPLETED_BIONIC handoff PASS"
else
  fail "17.A inherited COMPLETED_BIONIC handoff (rc=${rc})"
...
if find "$hop_a/opt/aelladata/os-upgrade/offline/backups" -path '*/handoff-state-*/state' -print0 2>/dev/null \
  | xargs -0 grep -l 'COMPLETED_BIONIC' >/dev/null 2>&1; then
  pass "17.A previous COMPLETED_BIONIC preserved in handoff backup"
else
...
if [[ "$rc" -ne 23 ]] && ! grep -q "unrecognized/corrupt state='COMPLETED_BIONIC'" "$hop_b/out.txt" \
  ...
  pass "17.B prior COMPLETED_BIONIC corrupt misclassification fixed"
else
  fail "17.B COMPLETED_BIONIC still treated as corrupt (rc=${rc})"
...
if _hop_buf_has . "$_hop_tmp"; then
  ...
  pass "17.B no corrupt-state backup for valid COMPLETED_BIONIC"
fi

# 17.C wrong OS (20.04 + COMPLETED_BIONIC)
...
if [[ "$rc" -ne 0 ]] && grep -qE 'HOP_HANDOFF_VALIDATION=FAIL|FAIL_PREVIOUS_HOP_HANDOFF_REJECTED|OS_VERSION_MISMATCH' "$hop_c/out.txt" \
  && [[ "$(cat "$hop_c/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_BIONIC" ]]; then
  pass "17.C COMPLETED_BIONIC on Focal refused; state preserved"
else
...
if [[ "$rc" -ne 0 ]] && grep -qE 'HOP_HANDOFF_VALIDATION=FAIL|OS_CODENAME_MISMATCH|FAIL_PREVIOUS_HOP_HANDOFF_REJECTED' "$hop_d/out.txt" \
  && [[ "$(cat "$hop_d/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_BIONIC" ]]; then
...
if [[ "$rc" -ne 0 ]] && grep -qE 'UPGRADE_ALREADY_RUNNING|duplicate execution refused|CURRENT_HOP_RUNNER_ACTIVE|FAIL_PREVIOUS_HOP_HANDOFF' "$hop_e/out.txt" \
  && [[ "$(cat "$hop_e/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_BIONIC" ]]; then
...
if [[ "$rc" -ne 0 ]] && grep -qE 'HOP_HANDOFF_VALIDATION=FAIL|DPKG_OR_APT_UNHEALTHY|dpkg lock held|FAIL_PREVIOUS_HOP_HANDOFF' "$hop_f/out.txt" \
  && [[ "$(cat "$hop_f/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_BIONIC" ]]; then
...
if [[ "$rc" -ne 0 ]] && grep -qE 'HOP_HANDOFF_VALIDATION=FAIL|DPKG_OR_APT_UNHEALTHY|FAIL_PREVIOUS_HOP_HANDOFF' "$hop_g/out.txt" \
  && [[ "$(cat "$hop_g/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_BIONIC" ]]; then
...
if [[ "$rc" -ne 0 ]] && grep -qE 'HOP_HANDOFF_VALIDATION=FAIL|AMBIGUOUS_MUTATION_EVIDENCE|CURRENT_HOP_MUTATION_EVIDENCE|FAIL_PREVIOUS_HOP_HANDOFF' "$hop_h/out.txt" \
  && [[ "$(cat "$hop_h/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_BIONIC" ]]; then
...
# 17.J COMPLETED_FOCAL on Bionic is contradiction (not inherited)
...
if [[ "$rc" -ne 0 ]] && grep -q 'FAIL_STATE_OS_CONTRADICTION' "$hop_j/out.txt" \
  ...
  && [[ "$(cat "$hop_j/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_FOCAL" ]]; then
  pass "17.J COMPLETED_FOCAL on Bionic refused (not inherited)"
else
  fail "17.J COMPLETED_FOCAL contradiction not enforced (rc=${rc})"
  tail -40 "$hop_j/out.txt" || true
fi

# 17.K Xenial→Bionic regression: COMPLETED_BIONIC remains terminal success (no b2f handoff)
X2B_IN="${ROOT}/client/dp-offline-upgrade-xenial-to-bionic.sh.in"
if [[ -f "$X2B_IN" ]]; then
  if grep -q 'already COMPLETED_BIONIC' "$X2B_IN" \
    && ! grep -q 'accept_previous_hop_terminal_state' "$X2B_IN" \
    && ! grep -q 'BIONIC_TO_FOCAL_HANDOFF_ACCEPTED' "$X2B_IN"; then
    pass "17.K x2b has no b2f handoff logic"
  else
    fail "17.K x2b unexpectedly gained b2f handoff logic"
  fi
  X2B_STUB="${OUT_DIR}/x2b-hop-stub.sh"
  python3 - "$X2B_IN" "$X2B_STUB" <<'PY'
import re, sys
src, dst = sys.argv[1], sys.argv[2]
from pathlib import Path
text = open(src, encoding="utf-8").read()
...
printf 'root:x:0:0:root:/root:/bin/bash\naella:x:1000:1000:aella:/home/aella:/bin/bash\n' >"$hop_k/etc/passwd"
  ...
  if [[ "$rc" -eq 0 ]] && grep -q 'already COMPLETED_BIONIC' "$hop_k/out.txt" \
    && ! grep -q 'BIONIC_TO_FOCAL_HANDOFF_ACCEPTED' "$hop_k/out.txt" \
    && [[ "$(cat "$hop_k/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_BIONIC" ]]; then
    pass "17.K x2b COMPLETED_BIONIC remains terminal (no handoff)"
  else
    fail "17.K x2b regression (rc=${rc})"
    tail -40 "$hop_k/out.txt" || true
  fi
else
  fail "17.K x2b template missing"
fi
...
if [[ "$rc1" -eq 0 && "$rc2" -eq 0 && "$handoff_count" -eq 1 && "$backup_count" -eq 1 ]] \
  ...
  && ! grep -q 'BIONIC_TO_FOCAL_HANDOFF_ACCEPTED=YES' "$hop_l/out2.txt"; then
...
# 17.L2 interruption: backup exists, live state still COMPLETED_BIONIC → safe re-eval
...
if [[ "$rc" -eq 0 ]] && grep -q 'HOP_HANDOFF_ACCEPTED=YES' "$hop_l2/out.txt" \
  && [[ "$(cat "$hop_l2/opt/aelladata/os-upgrade/offline/state")" != "COMPLETED_BIONIC" ]]; then
...
# 17.M quarantined backup recovery (blank live state + corrupt-state COMPLETED_BIONIC)
...
if [[ "$rc" -eq 0 ]] \
  ...
  pass "17.M quarantine COMPLETED_BIONIC recovery/handoff PASS"
else
  fail "17.M quarantine recovery (rc=${rc})"
  tail -50 "$hop_m/out.txt" || true
fi
# Refuse blind trust: quarantine COMPLETED_BIONIC but mutation evidence present
...
if [[ "$rc" -eq 0 ]] \
  ...
  && grep -q 'PREVIOUS_HOP_RUNTIME_OWNER=xenial-to-bionic' "$hop_n/out.txt" \
...
if [[ "$rc" -ne 0 ]] && grep -qE 'AMBIGUOUS_MUTATION_EVIDENCE|HOP_HANDOFF_VALIDATION=FAIL' "$hop_p/out.txt" \
  && [[ "$(cat "$hop_p/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_BIONIC" ]]; then
...
# 17.S startup-order: COMPLETED_BIONIC does not load old markers before handoff
...
# 17.U interruption before archive complete → state stays COMPLETED_BIONIC, retryable
...
if [[ "$rc" -eq 0 ]] && grep -qE 'HOP_HANDOFF_RESUME_ARCHIVE|HOP_HANDOFF_ACCEPTED=YES' "$hop_u/out.txt" \
  && [[ "$(cat "$hop_u/opt/aelladata/os-upgrade/offline/state")" != "COMPLETED_BIONIC" ]]; then
...
set -e
if [[ "$rc" -eq 0 ]] \
  && ! grep -q 'BIONIC_TO_FOCAL_HANDOFF_ACCEPTED=YES' "$hop_w/out.txt" \
...
if [[ "$rc1" -eq 0 && "$rc2" -eq 0 && "$bc" -eq 1 ]] \
  && grep -q 'BIONIC_TO_FOCAL_HANDOFF_ACCEPTED=YES' "$hop_x/out1.txt" \
  && ! grep -q 'BIONIC_TO_FOCAL_HANDOFF_ACCEPTED=YES' "$hop_x/out2.txt"; then
...
# 17.Y x2b regression already covered by 17.K - reinforce presence check
if grep -q 'already COMPLETED_BIONIC' "${ROOT}/client/dp-offline-upgrade-xenial-to-bionic.sh.in" \
  && ! grep -q 'classify_mutation_evidence_ownership' "${ROOT}/client/dp-offline-upgrade-xenial-to-bionic.sh.in"; then
  pass "17.Y x2b COMPLETED_BIONIC terminal unchanged"
else
  fail "17.Y x2b regression"
fi

# 17.Z Focal contradiction (20.04 + COMPLETED_BIONIC) - reinforce 17.C
...
if [[ "$rc" -ne 0 ]] && grep -qE 'OS_VERSION_MISMATCH|FAIL_PREVIOUS_HOP_HANDOFF' "$hop_z/out.txt"; then
  pass "17.Z Focal contradiction fail-closed"
else
  fail "17.Z Focal contradiction (rc=${rc})"
...
if [[ "$rc" -eq 0 ]] \
  ...
  && grep -q 'CURRENT_HOP=bionic-to-focal' "$hop_aa/opt/aelladata/os-upgrade/offline/current-hop.env" \
...
STELLAR_EXPECTED_HOP=xenial-to-bionic
STELLAR_EXPECTED_MIRROR_URI=http://127.0.0.1/hops/xenial-to-bionic/ubuntu
EOF
...
printf 'STELLAR_EXPECTED_HOP=xenial-to-bionic\n' \
...
printf 'previous_hop=xenial-to-bionic\n' \
...
if python3 - <<'PY'
from pathlib import Path
text = Path("/home/aella/ubuntu-mirror-automation/client/dp-offline-upgrade-bionic-to-focal.sh.in").read_text(encoding="utf-8")
...
X2B_IN="${ROOT}/client/dp-offline-upgrade-xenial-to-bionic.sh.in"
if grep -q 'install_effective_source_gate()' "$X2B_IN" \
  && grep -q 'SOURCE_GATE_INSTALL=PASS\|DISTUPGRADE_EFFECTIVE_SOURCE_GATE_STATE=INSTALLED_DISARMED' "$X2B_IN" \
  && grep -q 'BINARIES_NOT_READY_BEFORE_HOOK' "$X2B_IN" \
  && ! grep -q 'accept_previous_hop_terminal_state' "$X2B_IN"; then
  pass "17.AP x2b source-gate install regression OK"
else
  fail "17.AP x2b source-gate regression"
fi
...
if [[ "$rc" -eq 0 ]] \
  ...
  pass "17.AQ fresh Bionic VM preflight PASS"
else
...
printf '%s\t%s\n' \
  'http://127.0.0.1:9/client/bionic-to-focal/meta-release-lts' '404' \
...
if [[ "$rc" -eq 0 ]] \
  ...
  && ! grep -q 'UPGRADE-BIONIC-TO-FOCAL' "$hop_ba/out.txt" \
...
# 17.BB completed Focal regression
...
if [[ "$rc" -eq 0 ]] && grep -q 'already COMPLETED_FOCAL' "$hop_bb/out.txt"; then
  pass "17.BB completed Focal exit 0"
else
  fail "17.BB completed Focal (rc=${rc})"
...
if grep -q 'READY_FOR_CONFIRMATION' "$hop_bc/out.txt" \
  && grep -qE 'UPGRADE-BIONIC-TO-FOCAL|DESTRUCTIVE OFFLINE OS UPGRADE|Type the confirmation' "$hop_bc/out.txt"; then
...
if [[ -f "${ROOT}/client/dp-offline-upgrade-bionic-to-focal.sh" ]]; then
  gen="${ROOT}/client/dp-offline-upgrade-bionic-to-focal.sh"
  bd_ok=1
  for marker in \
...
if grep -q 'HTTP_SHA256\|ARTIFACT_SHA256\|HTTP_SIGNATURE_VERIFY' \
  "${ROOT}/scripts/deploy-client-bionic-to-focal-atomic.sh"; then
...
# =============================================================================
# 19) B2F atomic current-hop identity contract (parity with F2J handoff fix)
# =============================================================================
B2F_ID_HARNESS="${OUT_DIR}/b2f-identity-harness.sh"
{
  cat <<'EOS'
#!/usr/bin/env bash
set -euo pipefail
...
PREVIOUS_HOP_TERMINAL_STATE="COMPLETED_BIONIC"
PREVIOUS_HOP_PIN_NAME="xenial-to-bionic"
STATE_ROOT="/opt/aelladata/os-upgrade/offline"
...
bash -n "$B2F_ID_HARNESS" && pass "19.B2F identity harness bash -n" || fail "19.B2F identity harness bash -n"

assert_b2f_eight_fields() {
  local envf="$1" sha="${2:-abc123}"
  grep -qx 'CURRENT_HOP=bionic-to-focal' "$envf" \
    && grep -qx 'CURRENT_SOURCE_VERSION=18.04' "$envf" \
    && grep -qx 'CURRENT_SOURCE_CODENAME=bionic' "$envf" \
    && grep -qx 'CURRENT_TARGET_VERSION=20.04' "$envf" \
    && grep -qx 'CURRENT_TARGET_CODENAME=focal' "$envf" \
    && grep -qx 'HANDOFF_SOURCE_STATE=COMPLETED_BIONIC' "$envf" \
    && grep -qx 'PACKAGE_TRANSITION_STARTED=false' "$envf" \
    && grep -qx "CLIENT_ARTIFACT_SHA256=${sha}" "$envf"
}

# 19.A full-field success + existing-file rewrite
hop_b2f_a="$(mktemp -d "${OUT_DIR}/hop-b2f-a.XXXX")"
mkdir -p "$hop_b2f_a/opt/aelladata/os-upgrade/offline"
cat >"$hop_b2f_a/opt/aelladata/os-upgrade/offline/current-hop.env" <<'EOF'
CURRENT_HOP=xenial-to-bionic
CURRENT_SOURCE_VERSION=16.04
CURRENT_SOURCE_CODENAME=xenial
CURRENT_TARGET_VERSION=18.04
CURRENT_TARGET_CODENAME=bionic
HANDOFF_SOURCE_STATE=COMPLETED_XENIAL
PACKAGE_TRANSITION_STARTED=true
CLIENT_ARTIFACT_SHA256=deadbeef
EOF
set +e
(
  export DP_OFFLINE_TEST_ROOT="$hop_b2f_a" TEST_ROOT="$hop_b2f_a"
  # shellcheck disable=SC1090
  source "$B2F_ID_HARNESS"
  initialize_current_hop_identity
) >"$hop_b2f_a/out.txt" 2>&1
rc=$?
set -e
if [[ "$rc" -eq 0 ]] \
  && grep -q 'CURRENT_HOP_IDENTITY_INITIALIZED=YES' "$hop_b2f_a/out.txt" \
  && assert_b2f_eight_fields "$hop_b2f_a/opt/aelladata/os-upgrade/offline/current-hop.env"; then
  pass "19.A B2F full-field success + rewrite"
else
  fail "19.A B2F full-field (rc=${rc})"
  cat "$hop_b2f_a/out.txt" || true
fi

# 19.B SHA post-write mismatch
hop_b2f_b="$(mktemp -d "${OUT_DIR}/hop-b2f-b.XXXX")"
mkdir -p "$hop_b2f_b/opt/aelladata/os-upgrade/offline" "$hop_b2f_b/bin"
cat >"$hop_b2f_b/bin/mv" <<'EOF'
#!/usr/bin/env bash
/bin/mv "$@"
rc=$?
for a in "$@"; do
  if [[ "$a" == *current-hop.env && -f "$a" ]]; then
    sed -i 's/^CLIENT_ARTIFACT_SHA256=.*/CLIENT_ARTIFACT_SHA256=tampered/' "$a"
  fi
done
exit "$rc"
EOF
chmod +x "$hop_b2f_b/bin/mv"
set +e
(
  export DP_OFFLINE_TEST_ROOT="$hop_b2f_b" TEST_ROOT="$hop_b2f_b"
  export PATH="$hop_b2f_b/bin:$PATH"
  # shellcheck disable=SC1090
  source "$B2F_ID_HARNESS"
  initialize_current_hop_identity
) >"$hop_b2f_b/out.txt" 2>&1
rc=$?
set -e
if [[ "$rc" -ne 0 ]] \
  && grep -q 'CURRENT_HOP_IDENTITY_INITIALIZED=NO' "$hop_b2f_b/out.txt" \
  && ! grep -q 'CURRENT_HOP_IDENTITY_INITIALIZED=YES' "$hop_b2f_b/out.txt" \
  && ! grep -q 'HOP_HANDOFF_ACCEPTED=YES' "$hop_b2f_b/out.txt"; then
  pass "19.B B2F SHA post-write mismatch fails closed"
else
  fail "19.B B2F SHA mismatch (rc=${rc})"
  cat "$hop_b2f_b/out.txt" || true
fi

# 19.C event ordering static
...
if "HOP_HANDOFF_ACCEPTED=YES" in pre:
    raise SystemExit("ACCEPTED=YES before initialize")
if "HOP_HANDOFF_STATUS=PENDING" not in pre:
    raise SystemExit("PENDING missing before initialize")
post = body[init_at:]
...
if "PERSISTED_CLIENT_ARTIFACT_SHA256=" not in post:
    raise SystemExit("persisted SHA missing in accepted event")
print("ORDER_OK")
PY
)" && [[ "$ORDER_OUT" == "ORDER_OK" ]]; then
  pass "19.C B2F handoff-event ACCEPTED after identity verify"
else
...
echo "SOME dp-offline-upgrade-bionic-to-focal CHECKS FAILED"
exit 1
