#!/usr/bin/env bash
# tests/test_dp_offline_upgrade_focal_to_jammy.sh
set -euo pipefail
...
SCRIPT_IN_RAW="${ROOT}/client/dp-offline-upgrade-focal-to-jammy.sh.in"
BUILD_PY="${ROOT}/scripts/lib/build_client_focal_to_jammy.py"
OUT_DIR="$(mktemp -d)"
...
STUB="${OUT_DIR}/dp-offline-upgrade-focal-to-jammy.stub.sh"
...
pins = {
    ...
    "HOP": "focal-to-jammy",
    "SOURCE_CODENAME": "focal",
    "TARGET_CODENAME": "jammy",
    "SOURCE_VERSION": "20.04",
    "TARGET_VERSION": "22.04",
    "COMPONENTS": "main universe",
    "SOURCE_SUITES": "focal",
    "TARGET_SUITES": "jammy jammy-updates jammy-security jammy-backports",
    ...
    "CONFIRM_PHRASE": "UPGRADE-FOCAL-TO-JAMMY",
...
# Wrong OS → FAIL
rm -rf "$fx2/opt/aelladata/os-upgrade"
sed -i 's/20.04/22.04/;s/focal/jammy/' "$fx2/etc/os-release"
rc="$(run_preflight_fixture "$fx2")"
...
run_commit_fixture() {
  ...
  rm -f "$root/etc/apt/trusted.gpg.d/stellar-offline-focal-to-jammy.gpg"
  ...
  set +e
  env DP_OFFLINE_TEST_ROOT="$root" DP_OFFLINE_FAKE_CONFIRM=UPGRADE-FOCAL-TO-JAMMY \
...
open(p, "wb").write(b"\xef\xbb\xbf# focal \xe2\x86\x92 focal\n" + b"".join(deb))
PY
set +e
...
printf 'deb [arch=amd64 signed-by=/k.gpg] http://m/ubuntu focal main universe\n' >"$fx_enc/signed.sources"
printf 'deb [arch=amd64 signed-by=/k.gpg] http://m/ubuntu focal-updates main universe\n' >>"$fx_enc/signed.sources"
printf 'deb [arch=amd64 signed-by=/k.gpg] http://m/ubuntu focal-security main universe\n' >>"$fx_enc/signed.sources"
printf 'deb [arch=amd64 signed-by=/k.gpg] http://m/ubuntu focal-backports main universe\n' >>"$fx_enc/signed.sources"
set +e
...
# 6) trusted=yes FAIL
printf 'deb [arch=amd64 trusted=yes] http://m/ubuntu focal main universe\n' >"$fx_enc/trusted.sources"
set +e
...
open("$fx_enc/bad.sources", "wb").write(
    b"# bad \xe2\n"
    b"deb [arch=amd64] http://m/ubuntu focal main universe\n"
)
PY
set +e
...
printf 'deb http://ppa.example/ubuntu focal main\n' >"$fx_rb/etc/apt/sources.list.d/example-ppa.list"
...
# Simulate mutation
printf 'deb [arch=amd64] http://offline/ubuntu focal main\n' >"$fx_rb/etc/apt/sources.list"
...
is_jammy_version_for_pkg() { return 1; }
assert_ascii_file() {
  local path="$1"
...
printf '# Managed by stellar offline focal\xe2\x86\x92bionic upgrade\n[METARELEASE]\nURI = file:///tmp/x\nURI_LTS = file:///tmp/x\n' \
...
# 2) ASCII focal-to-jammy -> PASS
meta_local="$fx_meta/opt/aelladata/os-upgrade/offline/meta-release-lts.runtime"
printf 'Dist: focal\nName: Ubuntu 22.04 LTS\n' >"$meta_local"
render_update_manager_meta_release "$meta_local" "$fx_meta/etc/update-manager/meta-release.good" "focal-to-jammy"
if assert_ascii_file "$fx_meta/etc/update-manager/meta-release.good"; then
  pass "ASCII focal-to-jammy meta-release -> PASS"
else
  fail "ASCII meta-release rejected"
fi
if grep -q 'focal-to-jammy' "$fx_meta/etc/update-manager/meta-release.good" \
...
# 12) OS 20.04 + clean -> partial NO
PARTIAL_RELEASE_TRANSITION="YES"
if ! package_transition_evidence_present; then
  PARTIAL_RELEASE_TRANSITION="NO"
  pass "OS 20.04 clean dpkg -> partial transition NO"
else
...
if grep -n "Managed by stellar offline focal" "$SCRIPT_IN" | grep -q $'\xe2'; then
...
if [[ -f "${SEL_ROOT}/keys/ubuntu-mirror-selective.gpg" \
  ...
  if [[ -f "${SEL_ROOT}/hops/focal-to-jammy/ubuntu/dists/focal/Release" ]] \
    ...
    if [[ "$rc" -eq 0 ]]; then
      ...
      BUILT="${OUT_DIR}/dp-offline-upgrade-focal-to-jammy.sh"
      ...
      grep -q 'trusted.gpg.d/stellar-offline-focal-to-jammy.gpg' "$BUILT" \
      ...
      grep -q 'UPGRADE-FOCAL-TO-JAMMY' "$BUILT" && pass "confirm phrase pinned" || fail "confirm phrase"
      grep -q 'DistUpgradeViewNonInteractive' "$BUILT" && pass "DRO frontend pinned" || fail "DRO frontend"
      grep -q 'COMPLETED_JAMMY' "$BUILT" && pass "COMPLETED_JAMMY state" || fail "COMPLETED_JAMMY"
      ...
      disc_sha="$(sha256sum "${ROOT}/artifacts/upgrade-discovery/focal-to-jammy/export-summary.json" | awk '{print $1}')"
      sleep 0
      disc_sha2="$(sha256sum "${ROOT}/artifacts/upgrade-discovery/focal-to-jammy/export-summary.json" | awk '{print $1}')"
...
EOF
  ...
  # Wrong OS
  sed -i 's/20.04/22.04/;s/focal/jammy/' "$fake/etc/os-release"
  set +e
  ...
  rc=$?
  set -e
  [[ "$rc" -ne 0 ]] && pass "reject non-focal OS" || fail "accepted non-focal OS"
  # restore focal
  cat >"$fake/etc/os-release" <<EOF
NAME="Ubuntu"
VERSION_ID="20.04"
...
EOF
  ...
  set +e
  DP_OFFLINE_TEST_ROOT="$fake" DP_OFFLINE_FAKE_DP_VERSION=6.2.0 DP_OFFLINE_FAKE_ROLE=AIO \
    DP_OFFLINE_FAKE_CONFIRM=UPGRADE-FOCAL-TO-JAMMY \
  ...
  if [[ -f "$fake/etc/apt/trusted.gpg.d/stellar-offline-focal-to-jammy.gpg" ]] \
    ...
    if [[ -f "$fake/etc/update-manager/meta-release" ]] \
       ...
       && grep -q 'focal-to-jammy' "$fake/etc/update-manager/meta-release" \
  ...
  if [[ -d "$fake/opt/aelladata/os-upgrade/offline" ]]; then
    printf 'UPGRADING_FOCAL_TO_JAMMY\n' >"$fake/opt/aelladata/os-upgrade/offline/state"
    set +e
    DP_OFFLINE_TEST_ROOT="$fake" DP_OFFLINE_FAKE_DP_VERSION=6.2.0 DP_OFFLINE_FAKE_ROLE=AIO \
...
EOF
  ...
  rc=$?
  set -e
  [[ "$rc" -eq 0 ]] && pass "COMPLETED_JAMMY idempotent exit 0" || fail "COMPLETED_JAMMY rerun failed"

  # Refuse 24.04+ (Jammy is this hop's target; later hops are out of scope)
  cat >"$fake/etc/os-release" <<EOF
NAME="Ubuntu"
VERSION_ID="24.04"
...
# 12.4 state PREPARING_FOCAL → PASS
hf="$(mktemp -d)"
make_handoff_fixture "$hf"
install_fake_systemctl "$hf" ok
rpid="$(spawn_fake_runner "$hf")"
printf 'PREPARING_FOCAL\n' >"$hf/opt/aelladata/os-upgrade/offline/state"
export DP_OFFLINE_TEST_ROOT="$hf" DP_OFFLINE_TEST_HANDOFF=1 TEST_ROOT="$hf" STELLAR_OFFLINE_TEST_ROOT="$hf"
...
[[ "$rc" -eq 0 ]] && grep -q 'UPGRADE_HANDOFF=PASS' "$hf/out-prep.txt" \
  && pass "state PREPARING_FOCAL → handoff PASS" \
  || { fail "PREPARING_FOCAL handoff"; cat "$hf/out-prep.txt"; }
kill_fake_runner "$hf"; rm -rf "$hf"
...
if _hop_buf_has_f 'Type=oneshot' "$_unit_blk" \
  ...
  _hop_tmp="$(_hop_capture grep -A25 'Description=Stellar offline OS upgrade Focal to Jammy' "$SCRIPT_IN")"
...
if [[ "$rc" -eq 0 ]] \
   ...
   && grep -q 'UPGRADING_FOCAL_TO_JAMMY' "$hf/out-mon.txt" \
...
printf 'deb http://example/ubuntu focal main\n' >"$hf/etc/apt/sources.list"
...
make_runner_fixture() {
  ...
  printf 'deb [arch=amd64] http://192.0.2.10/hops/focal-to-jammy/ubuntu focal main universe\n' \
  ...
  # DistUpgrade target sources (4 jammy pockets)
  {
    echo 'deb [arch=amd64] http://192.0.2.10/hops/focal-to-jammy/ubuntu jammy main universe'
    echo 'deb [arch=amd64] http://192.0.2.10/hops/focal-to-jammy/ubuntu jammy-updates main universe'
    echo 'deb [arch=amd64] http://192.0.2.10/hops/focal-to-jammy/ubuntu jammy-security main universe'
    echo 'deb [arch=amd64] http://192.0.2.10/hops/focal-to-jammy/ubuntu jammy-backports main universe'
  } >"$root/opt/aelladata/os-upgrade/offline/distupgrade-target.sources.list"
  # Fake keyring file (gpg stub will report fingerprint)
  printf 'FAKEKEY\n' >"$root/etc/apt/trusted.gpg.d/stellar-offline-focal-to-jammy.gpg"
...
if [[ "$rc" -eq 0 ]] \
  ...
  && grep -q 'PIN_HOP=focal-to-jammy' "$rf/var/log/aella/offline_os_upgrade.log" \
...
if grep -q 'APT_SOURCES_LIST_RESTORED=PASS' "$logf" \
  ...
  && grep -q 'deb http://example.invalid/ focal main' "$rf/etc/apt/sources.list"; then
...
st="$(cat "$rf/opt/aelladata/os-upgrade/offline/state")"
[[ "$st" == "FAILED_PRE_DRO" ]] && pass "pre-DRO fail state=FAILED_PRE_DRO (not PREPARING_FOCAL)" \
...
# 13.10 stale PREPARING_FOCAL without live evidence → not MONITOR_ONLY
sf="$(mktemp -d)"
...
if grep -q 'STALE_STATE_DETECTED=YES' "$sf/out-stale.txt" \
  ...
  pass "stale PREPARING_FOCAL diagnosed; not MONITOR_ONLY"
else
  fail "stale PREPARING_FOCAL handling"
...
if grep -q "PIN_MIRROR_BASE='http://192.0.2.10'" "$envf" \
  && grep -q "PIN_HOP='focal-to-jammy'" "$envf" \
  && grep -q "PIN_TARGET_SUITES='jammy jammy-updates jammy-security jammy-backports'" "$envf" \
...
printf '# stellar offline DistUpgrade ValidMirrors overlay (Focal-to-Jammy)\nhttp://archive.ubuntu.com/ubuntu/\n' \
  >"$rf/opt/aelladata/os-upgrade/offline/distupgrade-valid-mirrors.cfg"
printf '# Generated by stellar offline Focal-to-Jammy client.\n[Sources]\nValidMirrors=%s\nAllowThirdParty=yes\n' \
  "$rf/opt/aelladata/os-upgrade/offline/distupgrade-valid-mirrors.cfg" \
...
if [[ "$st" == "FAILED_BEFORE_PACKAGE_TRANSITION" ]] \
  ...
  && grep -q 'deb http://example.invalid/ focal main' "$rf/etc/apt/sources.list" \
...
LOCAL_URI='http://192.0.2.10/hops/focal-to-jammy/ubuntu'
write_bionic_sources() {
  local dest="$1"
  cat >"$dest" <<EOF
deb [arch=amd64] ${LOCAL_URI} focal main universe
deb [arch=amd64] ${LOCAL_URI} focal-updates main universe
deb [arch=amd64] ${LOCAL_URI} focal-security main universe
deb [arch=amd64] ${LOCAL_URI} focal-backports main universe
EOF
}
write_focal_sources() {
  local dest="$1"
...
run_gate() {
  ...
  env -i PATH="/usr/bin:/bin" \
    ...
    STELLAR_TARGET_CODENAME=jammy \
    STELLAR_SOURCE_CODENAME=focal \
...
if [[ "$rc" -eq 0 ]] && printf '%s\n' "$out" | grep -q 'DISTUPGRADE_EFFECTIVE_SOURCE_GATE_STATE=DISARMED' \
  ...
  pass "DISARMED gate allows Focal (NOT_ARMED)"
else
  fail "DISARMED gate failed (rc=${rc})"
  printf '%s\n' "$out" || true
fi

# Arm marker + Focal only → DEFER
cat >"$GATE_FIX/opt/aelladata/os-upgrade/offline/effective-source-gate.armed" <<EOF
GATE_SCHEMA_VERSION=1
EXPECTED_SOURCE_CODENAME=focal
EXPECTED_TARGET_CODENAME=jammy
EXPECTED_MIRROR_URI=${LOCAL_URI}
EXPECTED_HOP=focal-to-jammy
...
if [[ "$rc" -eq 0 ]] && printf '%s\n' "$out" | grep -q 'ARMED_WAITING_FOR_TARGET_REWRITE' \
  ...
  pass "ARMED+Focal defers (TARGET_REWRITE_NOT_YET_VISIBLE)"
else
  fail "ARMED defer failed (rc=${rc})"
  printf '%s\n' "$out" || true
fi

# Arm + local Jammy → PASS + passed marker
write_focal_sources "$GATE_FIX/etc/apt/sources.list"
set +e
out="$(run_gate "$GATE_FIX/etc/apt/sources.list" 2>/dev/null)"
rc=$?
set -e
if [[ "$rc" -eq 0 ]] \
  ...
  pass "ARMED+local Jammy strict PASS + passed marker"
else
  fail "ARMED Jammy PASS failed (rc=${rc})"
  printf '%s\n' "$out" || true
fi
...
# Arm + country mirror
cat >"$GATE_FIX/etc/apt/sources.list" <<EOF
deb [arch=amd64] http://kr.archive.ubuntu.com/ubuntu jammy main universe
deb [arch=amd64] ${LOCAL_URI} jammy-updates main universe
deb [arch=amd64] ${LOCAL_URI} jammy-security main universe
deb [arch=amd64] ${LOCAL_URI} jammy-backports main universe
EOF
...
# Duplicate
write_focal_sources "$GATE_FIX/etc/apt/sources.list"
echo "deb [arch=amd64] ${LOCAL_URI} jammy main universe" >>"$GATE_FIX/etc/apt/sources.list"
set +e
out="$(run_gate "$GATE_FIX/etc/apt/sources.list" 2>/dev/null)"
rc=$?
set -e
...
open(bin_path + '.env', 'w').write(
    'STELLAR_EXPECTED_MIRROR_URI=http://192.0.2.10/hops/focal-to-jammy/ubuntu\n'
    'STELLAR_TARGET_CODENAME=jammy\n'
    'STELLAR_SOURCE_CODENAME=focal\n'
    'STELLAR_COMPONENTS=main universe\n'
...
# Arm only now (DRO eve), still Focal sources → next Pre-Invoke would DEFER.
...
# Simulate Pre-Invoke after arm with Focal sources still in place.
...
# =============================================================================
# 17) Previous-hop (COMPLETED_FOCAL) multi-hop handoff
# =============================================================================
...
grep -q 'FOCAL_TO_JAMMY_HANDOFF_ACCEPTED=YES' "$SCRIPT_IN" \
  && pass "FOCAL_TO_JAMMY_HANDOFF_ACCEPTED marker" || fail "handoff accepted marker missing"
_hop_tmp="$(_hop_capture grep -A3 'COMPLETED_FOCAL)' "$SCRIPT_IN")"
if _hop_buf_has_f 'accept_previous_hop_terminal_state' "$_hop_tmp"; then
  pass "COMPLETED_FOCAL routes through accept_previous_hop_terminal_state"
else
  fail "COMPLETED_FOCAL not routed through handoff accept"
fi
...
hop_fx_prep() {
  ...
  cat >"$root/etc/os-release" <<EOF
NAME="Ubuntu"
VERSION="20.04.6 LTS (Focal Beaver)"
ID=ubuntu
ID_LIKE=debian
...
# 17.A normal inherited COMPLETED_FOCAL → handoff → preflight
...
if [[ "$rc" -eq 0 ]] \
  ...
  && grep -q 'PREVIOUS_HOP_STATE=COMPLETED_FOCAL' "$hop_a/out.txt" \
  && ! grep -q "unrecognized/corrupt state='COMPLETED_FOCAL'" "$hop_a/out.txt" \
  ...
  pass "17.A inherited COMPLETED_FOCAL handoff PASS"
else
  fail "17.A inherited COMPLETED_FOCAL handoff (rc=${rc})"
  tail -50 "$hop_a/out.txt" || true
fi
_hop_tmp="$(_hop_capture find "$hop_a/opt/aelladata/os-upgrade/offline/backups" -type d -name 'handoff-state-*')"
if _hop_buf_has . "$_hop_tmp"; then
...
if find "$hop_a/opt/aelladata/os-upgrade/offline/backups" -path '*/handoff-state-*/state' -print0 2>/dev/null \
  | xargs -0 grep -l 'COMPLETED_FOCAL' >/dev/null 2>&1; then
  pass "17.A previous COMPLETED_FOCAL preserved in handoff backup"
else
...
rc=$?
set -e
if [[ "$rc" -ne 23 ]] && ! grep -q "unrecognized/corrupt state='COMPLETED_FOCAL'" "$hop_b/out.txt" \
  && grep -q 'HOP_HANDOFF_VALIDATION=PASS' "$hop_b/out.txt"; then
  pass "17.B prior COMPLETED_FOCAL corrupt misclassification fixed"
else
  fail "17.B COMPLETED_FOCAL still treated as corrupt (rc=${rc})"
  tail -40 "$hop_b/out.txt" || true
fi
_hop_tmp="$(_hop_capture find "$hop_b/opt/aelladata/os-upgrade/offline/backups" -type d -name 'corrupt-state-*')"
if _hop_buf_has . "$_hop_tmp"; then
  fail "17.B valid handoff incorrectly created corrupt-state backup"
else
  pass "17.B no corrupt-state backup for valid COMPLETED_FOCAL"
fi

# 17.C wrong OS (22.04 Jammy + COMPLETED_FOCAL)
...
if [[ "$rc" -ne 0 ]] && grep -qE 'HOP_HANDOFF_VALIDATION=FAIL|FAIL_PREVIOUS_HOP_HANDOFF_REJECTED|OS_VERSION_MISMATCH' "$hop_c/out.txt" \
  && [[ "$(cat "$hop_c/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_FOCAL" ]]; then
  pass "17.C COMPLETED_FOCAL on Jammy refused; state preserved"
else
...
if [[ "$rc" -ne 0 ]] && grep -qE 'HOP_HANDOFF_VALIDATION=FAIL|OS_CODENAME_MISMATCH|FAIL_PREVIOUS_HOP_HANDOFF_REJECTED' "$hop_d/out.txt" \
  && [[ "$(cat "$hop_d/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_FOCAL" ]]; then
...
if [[ "$rc" -ne 0 ]] && grep -qE 'UPGRADE_ALREADY_RUNNING|duplicate execution refused|CURRENT_HOP_RUNNER_ACTIVE|FAIL_PREVIOUS_HOP_HANDOFF' "$hop_e/out.txt" \
  && [[ "$(cat "$hop_e/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_FOCAL" ]]; then
...
if [[ "$rc" -ne 0 ]] && grep -qE 'HOP_HANDOFF_VALIDATION=FAIL|DPKG_OR_APT_UNHEALTHY|dpkg lock held|FAIL_PREVIOUS_HOP_HANDOFF' "$hop_f/out.txt" \
  && [[ "$(cat "$hop_f/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_FOCAL" ]]; then
...
if [[ "$rc" -ne 0 ]] && grep -qE 'HOP_HANDOFF_VALIDATION=FAIL|DPKG_OR_APT_UNHEALTHY|FAIL_PREVIOUS_HOP_HANDOFF' "$hop_g/out.txt" \
  && [[ "$(cat "$hop_g/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_FOCAL" ]]; then
...
if [[ "$rc" -ne 0 ]] && grep -qE 'HOP_HANDOFF_VALIDATION=FAIL|AMBIGUOUS_MUTATION_EVIDENCE|CURRENT_HOP_MUTATION_EVIDENCE|FAIL_PREVIOUS_HOP_HANDOFF' "$hop_h/out.txt" \
  && [[ "$(cat "$hop_h/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_FOCAL" ]]; then
...
rc=$?
set -e
_hop_tmp="$(_hop_capture find "$hop_i/opt/aelladata/os-upgrade/offline/backups" -type d -name 'corrupt-state-*')"
if [[ "$rc" -eq 23 ]] && grep -q "unrecognized/corrupt state='RANDOM_GARBAGE'" "$hop_i/out.txt" \
  && _hop_buf_has . "$_hop_tmp" \
...
# 17.J COMPLETED_JAMMY on Focal is contradiction (not inherited)
...
if [[ "$rc" -ne 0 ]] && grep -q 'FAIL_STATE_OS_CONTRADICTION' "$hop_j/out.txt" \
  ...
  && [[ "$(cat "$hop_j/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_JAMMY" ]]; then
  pass "17.J COMPLETED_JAMMY on Focal refused (not inherited)"
else
  fail "17.J COMPLETED_JAMMY contradiction not enforced (rc=${rc})"
  tail -40 "$hop_j/out.txt" || true
fi

# 17.K Bionic→Focal regression: COMPLETED_FOCAL remains terminal for B2F client (no F2J handoff)
B2F_IN="${ROOT}/client/dp-offline-upgrade-bionic-to-focal.sh.in"
if [[ -f "$B2F_IN" ]]; then
  if grep -q 'already COMPLETED_FOCAL' "$B2F_IN" \
    && ! grep -q 'FOCAL_TO_JAMMY_HANDOFF_ACCEPTED' "$B2F_IN"; then
    pass "17.K b2f has no f2j handoff logic"
  else
    fail "17.K b2f unexpectedly gained f2j handoff logic"
  fi
...
printf 'root:x:0:0:root:/root:/bin/bash\naella:x:1000:1000:aella:/home/aella:/bin/bash\n' >"$hop_k/etc/passwd"
  ...
  rc=$?
  set -e
  if [[ "$rc" -eq 0 ]] && grep -q 'already COMPLETED_FOCAL' "$hop_k/out.txt" \
    && ! grep -q 'FOCAL_TO_JAMMY_HANDOFF_ACCEPTED' "$hop_k/out.txt" \
    && [[ "$(cat "$hop_k/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_FOCAL" ]]; then
    pass "17.K b2f COMPLETED_FOCAL remains terminal (no f2j handoff)"
  else
...
rc2=$?
set -e
handoff_count="$(grep -c 'FOCAL_TO_JAMMY_HANDOFF_ACCEPTED=YES' "$hop_l/out1.txt" "$hop_l/out2.txt" 2>/dev/null | awk -F: '{s+=$2} END{print s+0}')"
...
if [[ "$rc1" -eq 0 && "$rc2" -eq 0 && "$handoff_count" -eq 1 && "$backup_count" -eq 1 ]] \
  ...
  && ! grep -q 'FOCAL_TO_JAMMY_HANDOFF_ACCEPTED=YES' "$hop_l/out2.txt"; then
...
# 17.L2 interruption: backup exists, live state still COMPLETED_FOCAL → safe re-eval
...
if [[ "$rc" -eq 0 ]] && grep -q 'HOP_HANDOFF_ACCEPTED=YES' "$hop_l2/out.txt" \
  && [[ "$(cat "$hop_l2/opt/aelladata/os-upgrade/offline/state")" != "COMPLETED_FOCAL" ]]; then
...
# 17.M quarantined backup recovery (blank live state + corrupt-state COMPLETED_FOCAL)
...
if [[ "$rc" -eq 0 ]] \
  ...
  pass "17.M quarantine COMPLETED_FOCAL recovery/handoff PASS"
else
  fail "17.M quarantine recovery (rc=${rc})"
  tail -50 "$hop_m/out.txt" || true
fi
# Refuse blind trust: quarantine COMPLETED_FOCAL but mutation evidence present
...
if [[ "$rc" -eq 0 ]] \
  ...
  && grep -q 'PREVIOUS_HOP_RUNTIME_OWNER=bionic-to-focal' "$hop_n/out.txt" \
...
if [[ "$rc" -ne 0 ]] && grep -qE 'AMBIGUOUS_MUTATION_EVIDENCE|HOP_HANDOFF_VALIDATION=FAIL' "$hop_p/out.txt" \
  && [[ "$(cat "$hop_p/opt/aelladata/os-upgrade/offline/state")" == "COMPLETED_FOCAL" ]]; then
...
# 17.S startup-order: COMPLETED_FOCAL does not load old markers before handoff
...
# 17.U interruption before archive complete → state stays COMPLETED_FOCAL, retryable
...
if [[ "$rc" -eq 0 ]] && grep -qE 'HOP_HANDOFF_RESUME_ARCHIVE|HOP_HANDOFF_ACCEPTED=YES' "$hop_u/out.txt" \
  && [[ "$(cat "$hop_u/opt/aelladata/os-upgrade/offline/state")" != "COMPLETED_FOCAL" ]]; then
...
set -e
if [[ "$rc" -eq 0 ]] \
  && ! grep -q 'FOCAL_TO_JAMMY_HANDOFF_ACCEPTED=YES' "$hop_w/out.txt" \
...
if [[ "$rc1" -eq 0 && "$rc2" -eq 0 && "$bc" -eq 1 ]] \
  && grep -q 'FOCAL_TO_JAMMY_HANDOFF_ACCEPTED=YES' "$hop_x/out1.txt" \
  && ! grep -q 'FOCAL_TO_JAMMY_HANDOFF_ACCEPTED=YES' "$hop_x/out2.txt"; then
...
if [[ "$rc" -ne 0 ]] && grep -qE 'OS_VERSION_MISMATCH|FAIL_PREVIOUS_HOP_HANDOFF' "$hop_z/out.txt"; then
  pass "17.Z Jammy contradiction fail-closed"
else
  fail "17.Z Jammy contradiction (rc=${rc})"
...
if [[ "$rc" -eq 0 ]] \
  ...
  && grep -q 'CURRENT_HOP=focal-to-jammy' "$hop_aa/opt/aelladata/os-upgrade/offline/current-hop.env" \
...
_hop_tmp="$(_hop_capture grep -A80 'ensure_source_gate_safe_for_preflight' "$SCRIPT_IN")"
_hop_tmp2="$(_hop_capture grep -B5 'ensure_source_gate_safe_for_preflight' "$SCRIPT_IN")"
if _hop_buf_has_f 'check_critical_package_holds' "$_hop_tmp" \
  || _hop_buf_has_f 'check_critical_package_holds' "$_hop_tmp2"; then
...
STELLAR_EXPECTED_HOP=bionic-to-focal
STELLAR_EXPECTED_MIRROR_URI=http://127.0.0.1/hops/bionic-to-focal/ubuntu
EOF
...
printf 'STELLAR_EXPECTED_HOP=bionic-to-focal\n' \
...
printf 'previous_hop=bionic-to-focal\n' \
...
# 17.AP b2f regression: source-gate install/validation still present; no F2J handoff
B2F_IN="${ROOT}/client/dp-offline-upgrade-bionic-to-focal.sh.in"
...
if [[ "$rc" -eq 0 ]] \
  ...
  pass "17.AQ fresh Focal VM preflight PASS"
else
...
printf '%s\t%s\n' \
  'http://127.0.0.1:9/client/focal-to-jammy/meta-release-lts' '404' \
...
# 17.BB completed Jammy regression
...
rc=$?
set -e
if [[ "$rc" -eq 0 ]] && grep -q 'already COMPLETED_JAMMY' "$hop_bb/out.txt"; then
  pass "17.BB completed Jammy exit 0"
else
  fail "17.BB completed Jammy (rc=${rc})"
...
if grep -q 'READY_FOR_CONFIRMATION' "$hop_bc/out.txt" \
  && grep -qE 'UPGRADE-FOCAL-TO-JAMMY|DESTRUCTIVE OFFLINE OS UPGRADE|Type the confirmation' "$hop_bc/out.txt"; then
...
if [[ -f "${ROOT}/client/dp-offline-upgrade-focal-to-jammy.sh" ]]; then
  gen="${ROOT}/client/dp-offline-upgrade-focal-to-jammy.sh"
  bd_ok=1
  for marker in \
...
if grep -q 'HTTP_SHA256\|ARTIFACT_SHA256\|HTTP_SIGNATURE_VERIFY' \
  "${ROOT}/scripts/deploy-client-focal-to-jammy-atomic.sh"; then
...
Package: lxd
Status: install ok installed
Version: 3.0.3-0ubuntu1~20.04.2
...
if grep -q 'CLIENT_EXIT_REASON=BACKGROUND_UPGRADE_FAILED' "$SCRIPT_IN" \
  ...
  && _hop_buf_has_f 'return "$mon_rc"' "$(_hop_capture grep -A80 '^start_upgrade_service_detached()' "$SCRIPT_IN")" \
  && _hop_buf_has_f 'handoff_rc' "$(_hop_capture grep -A120 '^commit_and_start()' "$SCRIPT_IN")" \
  && _hop_buf_has_f 'return "$rc"' "$(_hop_capture grep -A20 '^run_os_upgrade()' "$SCRIPT_IN")" \
  && _hop_buf_has_f 'upgrade_rc' "$(_hop_capture grep -A50 'run_os_upgrade "\$work"' "$SCRIPT_IN")"; then
...
# =============================================================================
# F2J Python2 policy (fixture / fake-class; no live apt/dpkg/snap mutation)
# =============================================================================
echo "======== F2J Python2 policy ========"
...
# 27/28) postboot: python2 residual ignored; python3 Jammy required
...
# F2J constant residual guard on template
if grep -nE 'UPGRADE-BIONIC-TO-FOCAL|UPGRADING_BIONIC_TO_FOCAL|PREPARING_BIONIC|COMPLETED_BIONIC|xenial-to-bionic|stellar-offline-bionic-to-focal|dp-offline-upgrade-bionic-to-focal' "$SCRIPT_IN"; then
  fail "F2J template contains B2F current-hop residuals"
else
  pass "F2J template has no B2F current-hop residuals"
fi
if grep -q 'bionic-to-focal' "$SCRIPT_IN" && grep -q 'PREVIOUS_HOP_PIN_NAME="bionic-to-focal"' "$SCRIPT_IN"; then
  pass "previous hop bionic-to-focal retained for handoff"
else
  fail "previous hop pin missing"
fi

# =============================================================================
# 28) F2J handoff identity / source-gate / transition-flag regressions (ops repro)
# =============================================================================
...
# --- 28.A operational reproduction: bionic current-hop.env + COMPLETED_FOCAL ---
...
# First invocation: empty confirmation → exit 21; identity must already be F2J
set +e
...
if [[ "$rc" -eq 21 ]] \
  ...
  && grep -q 'CURRENT_SOURCE_CODENAME=focal' "$hop_ops/opt/aelladata/os-upgrade/offline/current-hop.env" \
  && grep -q 'CURRENT_TARGET_CODENAME=jammy' "$hop_ops/opt/aelladata/os-upgrade/offline/current-hop.env" \
  && grep -q 'PACKAGE_TRANSITION_STARTED=false' "$hop_ops/opt/aelladata/os-upgrade/offline/current-hop.env" \
  && ! grep -q 'CURRENT_HOP=bionic-to-focal' "$hop_ops/opt/aelladata/os-upgrade/offline/current-hop.env" \
  ...
  pass "28.A1 ops repro: empty confirm exit=21 keeps F2J current-hop.env"
else
...
ID_HARNESS="${OUT_DIR}/f2j-identity-harness.sh"
{
  cat <<'EOS'
#!/usr/bin/env bash
set -euo pipefail
...
initialize_current_hop_identity_fail_mv() {
  ...
  if mv -f "$tmp" "$dest" 2>/dev/null; then
    ...
    if [[ -f "$dest" ]] && grep -q '^CURRENT_HOP=focal-to-jammy$' "$dest"; then
...
assert_f2j_eight_fields() {
  local envf="$1" sha="${2:-abc123}"
  grep -qx 'CURRENT_HOP=focal-to-jammy' "$envf" \
    && grep -qx 'CURRENT_SOURCE_VERSION=20.04' "$envf" \
    && grep -qx 'CURRENT_SOURCE_CODENAME=focal' "$envf" \
    && grep -qx 'CURRENT_TARGET_VERSION=22.04' "$envf" \
    && grep -qx 'CURRENT_TARGET_CODENAME=jammy' "$envf" \
    && grep -qx 'HANDOFF_SOURCE_STATE=COMPLETED_FOCAL' "$envf" \
...
if [[ "$rc" -eq 0 ]] \
  ...
  && grep -q 'PERSISTED_CURRENT_HOP=focal-to-jammy' "$hop_id/out-ok.txt" \
  && grep -q 'PERSISTED_CLIENT_ARTIFACT_SHA256=abc123' "$hop_id/out-ok.txt" \
  && assert_f2j_eight_fields "$hop_id/opt/aelladata/os-upgrade/offline/current-hop.env" \
  && ! grep -q 'CURRENT_HOP=bionic-to-focal' "$hop_id/opt/aelladata/os-upgrade/offline/current-hop.env"; then
...
SG_HARNESS="${OUT_DIR}/f2j-sg-harness.sh"
{
  cat <<'EOS'
#!/usr/bin/env bash
set -euo pipefail
...
if [[ "$rc" -eq 0 ]] \
  ...
  pass "28.C2 valid F2J identity → SOURCE_GATE_INSTALL=PASS"
else
...
if [[ "$rc" -eq 0 ]] \
  ...
  && assert_f2j_eight_fields "$hop_e2/opt/aelladata/os-upgrade/offline/current-hop.env" \
  && ! grep -q 'CURRENT_HOP=bionic-to-focal' "$hop_e2/opt/aelladata/os-upgrade/offline/current-hop.env" \
...
if [[ "$rc" -eq 21 ]] \
  ...
  && grep -qx 'PERSISTED_CURRENT_HOP=focal-to-jammy' "$evt" \
  && assert_f2j_eight_fields "$hop_e6/opt/aelladata/os-upgrade/offline/current-hop.env" "$ZERO_SHA"; then
...
event=FOCAL_TO_JAMMY_HANDOFF_PENDING
...
if [[ "$rc" -eq 0 ]] \
  ...
  && assert_f2j_eight_fields "$hop_e7/opt/aelladata/os-upgrade/offline/current-hop.env" \
  && ! grep -Fq 'skip-if-exists' "$hop_e7/out.txt"; then
  pass "28.E7/F existing bionic-to-focal env always rewritten to F2J contract"
else
...
if [[ "$FAIL" -eq 0 ]]; then
  echo "ALL dp-offline-upgrade-focal-to-jammy CHECKS PASSED"
  exit 0
fi
echo "SOME dp-offline-upgrade-focal-to-jammy CHECKS FAILED"
exit 1
